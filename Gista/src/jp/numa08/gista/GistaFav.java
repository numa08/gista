...
public class GistaFav {
	...
	public void fav(Gist gist) {
		final FavGistDB db = new FavGistDB(context);
		if (db.select(gist) != null) {
			// TODO: あんふぁぼ？
			Toast.makeText(context, "あんふぁぼる？", Toast.LENGTH_SHORT).show();
			return;
		}
		...
		if (now.getLevel() > old.getLevel()) {
			...
			Toast.makeText(context, String.format("%d経験値を取得した！！", exp),
					Toast.LENGTH_SHORT).show();
		}
